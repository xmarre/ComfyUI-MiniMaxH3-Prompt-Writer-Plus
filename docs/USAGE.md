...
Install [ComfyUI H3 Continuum](https://github.com/ukr8b3g-cmyk/ComfyUI-H3-Continuum) and use a supported **H3 Continuum Sampler V3.4–V3.7** (V3.7 is the current upstream release). Connect an editable STRING source such as **Text (Multiline)** or **State Manager Text Box** to the sampler's **Sequence Prompt** input. **Apply to Continuum** requires **Prompt Format = Timeline**. If Prompt Format, chunk count, or chunk duration differs, Writer shows every mismatch before resolving whether the connected text source is directly editable and offers **Sync settings & apply**. For an unmanaged State Manager Text Box, Writer updates its `text` widget directly. When that box has a connected `state_control`, State Manager owns the runtime text; Writer never overwrites the non-authoritative widget, synchronizes the confirmed sampler settings, and copies the canonical Timeline sequence so the authoritative State Manager prompt can be updated. Direct text-write failures restore the sampler settings and previous text. Writer does not add or rewire nodes.
