...
Install [ComfyUI H3 Continuum](https://github.com/ukr8b3g-cmyk/ComfyUI-H3-Continuum) and use a supported **H3 Continuum Sampler V3.4–V3.7** (V3.7 is the current upstream release). Connect an editable STRING source such as **Text (Multiline)** or **State Manager Text Box** to the sampler's **Sequence Prompt** input. **Apply to Continuum** accepts **Prompt Format = Auto** or **Timeline**. Auto is left unchanged and detects Writer's canonical Timeline at execution; Fixed/List are incompatible and **Sync settings & apply** changes them to Auto. Chunk-count and chunk-duration mismatches still require explicit synchronization. For an unmanaged State Manager Text Box, Writer updates its `text` widget directly. When that box has a connected `state_control`, State Manager owns the runtime text; Writer requires the managed prompt-document contract v5 and atomically saves both the canonical Timeline text and its explicit logical-chunk interpretation. The document records the exact chunk count and canonical decimal chunk duration, so execution does not have to infer intent from prose. A failed, stale, or incomplete persistent write aborts the handoff and restores sampler-setting mutations instead of reporting success. Writer's authoring range remains 4–30 seconds per chunk, but managed Apply intersects that with the selected sampler's advertised widget limits and refuses unsupported geometry without clamping; for the current H3 Continuum transport stack that execution range is 4–15 seconds. Writer does not add or rewire nodes.

A successful managed Apply confirms that State Manager persisted the text and interpretation descriptor. It does **not** by itself prove queue-time wildcard expansion or physical prompt compilation. At execution, State Manager freezes the selected revision, Impact applies its native wildcard semantics, and H3 Continuum validates the expanded structure before compiling physical/Qwen text. Runtime transport receipts are the evidence for that later boundary.
