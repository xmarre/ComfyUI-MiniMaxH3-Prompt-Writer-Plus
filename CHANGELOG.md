...
- Updated **Sync settings & apply** to validate Prompt Format, chunk count, and chunk duration before Sequence Prompt editability. Unmanaged State Manager Text Boxes are editable; boxes with connected `state_control` are runtime-managed and their non-authoritative text widget is never overwritten. Direct text-write failures still restore the sampler settings and previous text.
